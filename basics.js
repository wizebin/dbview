function showInID(obID,content){
	var elem = document.getElementById(obID)
	if (elem!=null){
...
var httpVERB = function(url, verb, params, username, password, successHandler, errorHandler) {
	...
	if (xhr.readyState == 4) {
	  ...
	  if (status == 200) {
		try{
			...
			console.log(err.message + " " + xhr.responseText);
			errorHandler && errorHandler("Error In httpVERB "+err.message+" DATA: "+xhr.responseText);
		}
		
	  } else {
		errorHandler && errorHandler("HTTP ERR: "+status);
	  }
	}
  };
  console.log("SENDING "+ verb + "  " + url + "?" + params);
  xhr.send(params);
};
